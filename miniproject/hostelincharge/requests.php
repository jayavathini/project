                     ...
                     <?php 
                          ...
                          if(isset($_POST['filter'])){
                               ...
                               if( $year!='YEAR' && $dept!='DEPARTMENT'){
                                 $sql=mysqli_query($con,"SELECT * FROM acceptrequest WHERE Year='$year' and Department='$dept'");
                               }
                               else if( $year!='YEAR'){
                                 $sql=mysqli_query($con,"SELECT * FROM acceptrequest WHERE Year='$year'");
                               }
                               else if($dept!='DEPARTMENT'){
                                 $sql=mysqli_query($con,"SELECT * FROM acceptrequest WHERE Department='$dept'");
                               }
                               else{
                                 $sql=mysqli_query($con,"SELECT * FROM acceptrequest");
                              }                     
                          }
                          else{
                             $sql=mysqli_query($con,"SELECT * FROM acceptrequest");
                     ...
                     <tbody>
                        <tr>
                            ...
                            <form action="acceptrequest.php" method="post">
                               ...
                               <input type="text" name="ra" value="<?php echo $row["Visit"];?>" hidden/>
                               <td class="edit"><input type="submit" value="accept" class="view-btn1" style="padding:10px 20px;font-size:17px;" /></td>
                            </form>
                            <form action="../reject.php" method="post">
                            <input type="text" name="gno" value="<?php echo $row["GatepassNo"];?>" hidden/>
                            <td class="edit" style="padding-right:40px;"><input type="submit" value="Reject" class="view-btn2" style="padding:10px 20px;font-size:17px;" /></td>
                           </form>
                        </tr>
                    </tbody>
                    <?php } ?>
